...
@endsection
@section("content")
<div class="w-full max-w-5xl relative mx-auto bg-white pt-20">
    ...
    <a href="/new-frequency">
        <button type="button" class='text-white bg-secondary-mid py-2 px-6 rounded'>Tambah Frekuensi kata</button>
    </a>
        ...
        <table class="border-collapse border border-is-gray-low table-auto w-full mt-8">
            <thead class="text-white">
                <tr>
                    ...
                    <th class="bg-secondary-mid border border-is-gray-low p-1 font-light text-center">Team</th>
                    <th class="bg-secondary-mid border border-is-gray-low p-1 font-light text-center">Jumlah</th>
                    <th class="bg-secondary-mid border border-is-gray-low p-1 font-light text-center">Kata</th>
            ...
            <tbody class="text-center">
                ...
                <tr class="text-sm font-light even:bg-white-low odd:bg-transparent">
                    ...
                    <td class="py-3 px-2 border border-is-gray-low text-black-low">{{ $data->team }}</td>
                    <td class="py-3 px-2 border border-is-gray-low text-black-low">{{ $data->amountword }}</td>
                    ...
                    <td class="py-3 px-2 border border-is-gray-low flex justify-center space-x-3 cursor-pointer">
                        <form action="{{route('del.freq', [$data->id])}}" method="POST">
                            @csrf
                            @method("DELETE")
                            <button type="submit" class="text-red-500 underline">Hapus data</button>
                        </form>
                        <a href="/freq-w/edit/{{$data->id}}" class="text-secondary-mid underline">Edit</a>
                    </td>
                </tr>
                @empty
